...
public class DeathCompassItem extends Item implements Vanishable {
   ...
   public static Optional<ItemStack> createDeathCompass(Player player) {
      ...
      if (optional.isPresent()) {
         ...
         if (capability.hasLastDeathData()) {
            ItemStack itemstack = new ItemStack(ModRegistry.DEATH_COMPASS_ITEM, 1);
   ...
   public void appendHoverText(ItemStack p_4itemStack457_, @Nullable Level level, List<Component> tooltip, TooltipFlag tooltipFlag) {
      if (p_4itemStack457_.hasTag()) {
         ...
         BlockPos blockpos = NbtUtils.readBlockPos(tag.getCompound("LastDeathPos"));
         tooltip.add(new TranslatableComponent("item.deathcompass.death_compass.tooltip.position", new TextComponent(String.valueOf(blockpos.getX())).withStyle(ChatFormatting.GRAY), new TextComponent(String.valueOf(blockpos.getY())).withStyle(ChatFormatting.GRAY), new TextComponent(String.valueOf(blockpos.getZ())).withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.GOLD));
         final Optional<ResourceKey<Level>> lastDeathDimension = getLastDeathDimension(tag);
         if (lastDeathDimension.isPresent()) {
            tooltip.add(new TranslatableComponent("item.deathcompass.death_compass.tooltip.dimension", new TextComponent(lastDeathDimension.get().location().toString()).withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.GOLD));
         }
         final long lastDeathDate = tag.getLong("LastDeathDate");
         if (lastDeathDate > 0) {
            String date = DATE_FORMAT.format(new Date(lastDeathDate));
            tooltip.add(new TranslatableComponent("item.deathcompass.death_compass.tooltip.date", new TextComponent(date).withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.GOLD));
         }
      }
   }
}